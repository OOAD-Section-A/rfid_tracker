 ...
 *                                 resolved_at
 *
 * SEPARATE FILE — DATABASE SUBSYSTEM TEAM may change credentials /
 * table names without touching service or UI code.
 * ═══════════════════════════════════════════════════════════════════
 */
public class DatabaseManager {
    ...
    // ── Connection config ─────────────────────────────────────────────────
    /** PRIMARY: shared OOAD database — run schema.sql first */
    private static final String DB_URL          = "jdbc:mysql://localhost:3306/OOAD"
                                                  + "?useSSL=false&allowPublicKeyRetrieval=true"
                                                  + "&serverTimezone=Asia/Kolkata";
    /** FALLBACK: original local RFID-only database (standalone demo/testing) */
    private static final String DB_URL_FALLBACK = "jdbc:mysql://localhost:3306/scm_rfid_db"
                                                  + "?useSSL=false&allowPublicKeyRetrieval=true"
                                                  + "&serverTimezone=Asia/Kolkata";
    ...
    private String activeDbUrl = DB_URL;
    ...
    // ── Connect — tries OOAD first, falls back to scm_rfid_db ─────────────
        ...
        try {
            if (connection == null || connection.isClosed()) {
                try {
                    ...
                    System.err.println("[DB] OOAD DB unavailable, trying fallback: "
    ...
    private String insertToSharedEventsTable(ScanRecord record) throws RFIDException {
        ...
        String resolvedProductId = record.getProductId() != null
                ? record.getProductId()
                : record.getRfidTag();

        if (barcodeAdapter != null) {
            // DB module is the single writer — no raw SQL insert alongside it.
            try {
                BarcodeRfidEvent evt = new BarcodeRfidEvent(
                    ...
                    "",   // category
                    "",   // description
                    eventId, // transactionId — use same eventId as reference
                    record.getTimestamp(),
                    record.getStatus(),
                    record.getSource()
                );
                barcodeAdapter.recordBarcodeEvent(evt);
                System.out.println("[DB] Scan event inserted via adapter: " + eventId);
            } catch (Exception e) {
                throw new RFIDException.DatabaseConnectionFailedException(
                        "DB module adapter failed to insert barcode_rfid_events: " + e.getMessage(), e);
            }
        } else {
            // Adapter unavailable (e.g. DB module JAR missing) — fall back to raw SQL.
            ...
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                ps.setString(2, resolvedProductId);
                ...
                ps.executeUpdate();
                System.out.println("[DB] Scan event inserted via raw SQL (adapter unavailable): " + eventId);
            } catch (SQLException e) {
                throw new RFIDException.DatabaseConnectionFailedException(
    ...
    public void logExceptionToSharedTable(RFIDException ex, String referenceId) {
        // Use the DB module's ExceptionHandlingSubsystemFacade when connected to OOAD.
        // Falls back to stderr only when offline or on legacy DB.
        if (scmFacade == null) {
            System.err.println("[DB] Exception not logged to shared table (adapter unavailable): "
        ...
        try {
            ...
            SubsystemException record = new SubsystemException(
                null,                           // id — auto-assigned by DB
                ex.getExceptionListId(),        // exceptionId from shared list
                ex.getErrorCode(),              // exceptionName
                ex.getCategory().name(),        // severity (MAJOR/MINOR/WARNING)
                "SUBSYSTEM_11_RFID_BARCODE",    // subsystem
                msg.length() > 500 ? msg.substring(0, 500) : msg,
                java.time.LocalDateTime.now()
            );
            scmFacade.exceptions().logException(record);
            System.out.println("[DB] Exception logged via ExceptionHandlingSubsystemFacade: "
                               + ex.getErrorCode());
        } catch (Exception ignored) {
            System.err.println("[DB] Could not write to subsystem_exceptions via facade: "
                               + ignored.getMessage());
        }
    }
